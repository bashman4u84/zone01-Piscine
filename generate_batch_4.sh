...
const checkpoints = [
    {
        title: "16. Fprime",
        question: "Write a program that takes a positive integer as a command-line argument and prints its prime factorization, followed by a newline. The factors should be listed in increasing order and separated by asterisks (*). For example, for the input 225, the output should be `3*3*5*5`.",
        answer: `package main
...
func main() {
	if len(os.Args) == 2 {
        ...
        explanation: `This program calculates and prints the prime factors of a given number.
...
3.  **Output**: The factors are printed sequentially, separated by asterisks, and a final newline is printed after the loop completes.`
    ...
    {
        title: "17. From To",
        question: "Write a function `FromTo` that takes two integers, `from` and `to`. It should return a string containing a sequence of numbers. If `from < to`, it's an increasing sequence. If `from > to`, it's a decreasing sequence. If `from == to`, it's just the number. All numbers in the sequence must be padded with a leading zero if they are less than 10. The numbers are separated by \", \". If any input is outside the range [0, 99], it should return \"Invalid\\n\".",
...
func FromTo(from, to int) string {
	...
	return result + "\\n"
        ...
        explanation: `This function generates a formatted string of numbers representing a range.
...
func main() {
	if len(os.Args) == 3 {
        ...
        explanation: `This program implements a text search based on a custom expression.
...
func main() {
	...
	fmt.Println("1")
        ...
        explanation: `This program checks if one string is an ordered subsequence of another.
...
4.  **Success**: If the loop completes, it means every character of \`s1\` was found in \`s2\` in the correct order. The program prints \`1\`.`
    ...
    {
        title: "20. Inter",
        question: "Write a program that takes two strings as command-line arguments. The program should find the characters that are common to both strings and print them, without duplicates, in the order they appear in the first string.",
        answer: `package main
...
func main() {
	if len(os.Args) == 3 {
        ...
        explanation: `This program finds the intersection of two strings, preserving the order of the first string and ensuring uniqueness.
...
function generateDoc() {
  const docChildren = [
    new Paragraph({
      text: "Checkpoint Solutions: Batch 4",
  ...
  Packer.toBuffer(doc).then((buffer) => {
    fs.writeFileSync("checkpoint_solutions_batch_4.docx", buffer);
    console.log("Successfully created checkpoint_solutions_batch_4.docx");
  });
}
...
chmod +x create_doc_batch_4.js
node create_doc_batch_4.js
